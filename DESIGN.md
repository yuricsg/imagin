...
- **Command palette** (`_components/command-palette.tsx`): overlay `role="dialog"` com input `role="combobox"` + lista `role="listbox"`, `aria-activedescendant`, ↑/↓/Enter/Esc, backdrop fecha, foco devolvido ao gatilho. Abre com **⌘K / Ctrl+K** (funciona mesmo dentro de inputs) ou pelo botão "⌘K" no header (evento `imagin:toggle-command-palette`; fora da home o botão vira link para "/"). Ações: criar chatbot, abrir cada bot, copiar snippet do bot selecionado (com aviso "Copiado!"), alternar tema, "Somente novos", exportar CSV.
...
- **Menu de ações do chatbot** (kebab "⋯", sempre visível em bots editáveis): padrão WAI-ARIA menu button — abre com clique/Enter/Espaço/↓, ↑↓ movem foco real, Esc fecha e devolve foco ao gatilho, clique fora fecha, um menu aberto por vez. Itens Editar/Duplicar + separador + Excluir (rose, com diálogo de confirmação). Dropdown entra com `.motion-menu` (fade + subida 4px, 150ms). Ações nunca ficam escondidas atrás de hover.
